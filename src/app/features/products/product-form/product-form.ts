...
export class ProductForm {
  ...
  // MAINTENANT : input() simple (pas required, car la création n'a pas d'id)
  id = input<string>();

  // isEditMode devient une méthode calculée, pas une propriété qu'on assigne manuellement
  ...
  constructor() {
    // remplace ngOnInit() — se déclenche dès que id() est disponible
    effect(() => {
      const currentId = this.id();
      if (currentId) {
        this.store.loadById(Number(currentId));
      }
    });

    // remplace ngDoCheck() — se déclenche quand selectedProduct() change
    effect(() => {
      const p = this.store.selectedProduct();
      if (this.isEditMode && p && !this.form.dirty) {
  ...
  get descriptionError(): string {
    ...
    if (ctrl?.touched && ctrl.hasError('minlength')) return 'La description doit comporter 10 caractères minimum';
    return '';
  }
  ...
  get categoryError(): string {
    ...
    if (ctrl?.touched && ctrl.hasError('required')) return 'Catégorie requise';
    return '';
  }
  ...
  onSubmit(): void {
    ...
    if (this.isEditMode) {
      ...
      this.toast.show('Produit modifié avec succès');
    } else {
